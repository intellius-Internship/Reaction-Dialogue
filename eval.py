...
def eval_ar(args, model, device, test_data):
    ...
    with torch.no_grad():
        for d in test_data.iterrows():
            ...
            # post processing
            try:
                # split system response into reply and reaction 
                responses = sys_response.split(DELIMITER)
                if len(responses) > 2:
                    reply = proc_reply(responses[0])
                    reaction = proc_reply(responses[-1])

            except Exception as e:
                # when an error occurs error, reply = reaction = 'None'
                # invalid prediction
                pass
        ...
        test_data['gen_reaction'] = reactions
...
Description
-----------
BART 기반 리액션 대화 모델 test data에서의 테스트
'''
...
Description
-----------
GPT2 기반 리액션 대화 모델 사용자 입력에 대한 테스트
'''
        ...
        while is_valid(query):
            ...
            try:
                ...
                if len(responses) > 2:
                    reply = proc_reply(responses[0])
                    reaction = proc_reply(responses[-1])
                    print("Reply: {} (CLASS: {})".format(reply, reaction))

            except Exception as e:
                # when an error occurs error, reply = reaction = 'None'
                # invalid prediction
                print("Fail: {}".format(sys_response))
                pass
...
Description
-----------
BART 기반 리액션 대화 모델 사용자 입력에 대한 테스트
'''
