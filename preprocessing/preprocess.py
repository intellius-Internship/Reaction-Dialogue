...
def get_balanced_dataset(data : pd.DataFrame, test_ratio : float, use_valid : bool):
    ...
    for idx in data.reaction_cls.unique().tolist():
        sub_data = data[data.reaction_cls==idx]
        ...
        if num_test == 0:
            if len(sub_data) < 2:
                ...
                # reaction class 내 데이터가 3-4개 인 경우
...
def split_dataset(args, data):
    ...
    print(f"Train Distribution : \n{train.reaction_cls.value_counts()}")
    print(f"Valid Distribution : \n{valid.reaction_cls.value_counts()}")
    print(f"Test Distribution : \n{test.reaction_cls.value_counts()}")
